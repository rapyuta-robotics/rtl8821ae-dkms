...
static u8 ReadAdapterInfo8812AE(PADAPTER Adapter)
{
...
#else
	rtw_write8(Adapter, REG_BCN_CTRL, 0x1d);
#endif
...
)
{
	...
	u8			numNQ = 0, numPubQ = 0, numHQ = 0, numLQ = 0, numEQ = 0;
	...
	*pRQPNValue = _HPQ(numHQ) | _LPQ(numLQ) | _PUBQ(numPubQ) | LD_RQPN;
	return (u8)(numHQ + numLQ + numPubQ + numNQ + numEQ + 1);
}
...
)
{
	...
	if (Adapter->registrypriv.wifi_spec)
		...
		if (IS_HARDWARE_TYPE_8812(Adapter))
			tmpU2b |= 0xF5B1;	/* HIQ->HPQ, MGQ->HPQ, BKQ->LPQ, BEQ->LPQ, VIQ->NPQ, VOQ->HPQ */
		else
...
static u32 rtl8812ae_hal_init(PADAPTER Adapter)
{
	...
	/* Init BT hw config. */
	if (IS_HARDWARE_TYPE_8812(Adapter)) {
		if (pHalData->rfe_type == 1 || pHalData->rfe_type == 5)
			phy_set_bb_reg(Adapter, r_ANTSEL_SW_Jaguar, 0x00000303, 0x0302);	/* 0x900[9,8,1,0] = b1110 */
	}
	...
	if (Adapter->registrypriv.mp_mode == 1) {
		...
		if (pHalData->eRFPowerState == eRfOn) {
			...
			odm_txpowertracking_check(&pHalData->DM_OutSrc);
...
)
{
	...
	if ((rtw_read8(Adapter, REG_MCUFWDL) & BIT7) && GET_HAL_DATA(Adapter)->bFWReady) /* 8051 RAM code */
...
u8 SetHwReg8812AE(PADAPTER Adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
	...
	case HW_VAR_SET_RPWM:
#ifdef CONFIG_LPS_LCLK
		{
			...
			rtw_write8(Adapter, REG_PCIE_HRPWM_8812A, ps_state);
		}
#endif
		break;
...
void GetHwReg8812AE(PADAPTER Adapter, u8 variable, u8 *val)
{
...
#ifdef CONFIG_LPS_LCLK
	case HW_VAR_CPWM:
		*val = rtw_read8(Adapter, REG_PCIE_HCPWM_8812A);
		break;
#endif

	case HW_VAR_DBI:
		*val = hal_dbi_read_8812ae(Adapter, *((u16 *)(val)));
		break;
	case HW_VAR_MDIO:
		*((u16 *)(val)) = hal_mdio_read_8812ae(Adapter, *val);
		break;
	case HW_VAR_L1OFF_NIC_SUPPORT:
		{
		u8 l1off;

		l1off = hal_dbi_read_8812ae(Adapter, 0x160);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
	...
	case HW_VAR_L1OFF_CAPABILITY:
		...
		l1off = hal_dbi_read_8812ae(Adapter, 0x15c);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
...
void rtl8812ae_set_hal_ops(_adapter *padapter)
{
	...
	pHalFunc->set_hw_reg_handler = &SetHwReg8812AE;
	pHalFunc->GetHwRegHandler = &GetHwReg8812AE;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8812AE;
